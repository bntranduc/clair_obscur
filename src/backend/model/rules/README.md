...
| `WEB_IDOR` | ≥10 GET 200 on API endpoints with numeric IDs from same IP | `application` |
...
| `KILL_CHAIN_WEBSCAN_TO_EXPLOIT` | Nikto/dir-bruteforce IP → exploitation rule (same window) |
...
- **`WEB_IDOR`** does not use a sliding window — API endpoints with numeric IDs are specific enough at threshold 10.
