...
from backend.agentic.config.config import MCPServerConfig

# Optional dependency: MCP support is only needed when mcp_servers are configured.
try:
    from fastmcp import Client  # type: ignore
    from fastmcp.client.transports import SSETransport, StdioTransport  # type: ignore
...
class MCPClient:
    ...
    def _create_transport(self) -> StdioTransport | SSETransport:
        if Client is None or SSETransport is None or StdioTransport is None:
            raise RuntimeError(
                "MCP support is not installed. Install `fastmcp` to enable MCP tools."
            )
