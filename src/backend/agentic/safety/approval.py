...
from backend.agentic.safety.plan_risk import PlanRiskAssessment, summarize_params
...
class ApprovalManager:
    def __init__(
        ...
        self._sse_bridge_queue: asyncio.Queue[tuple[str, str]] | None = None
        self.conversation_id: str | None = None
        self.web_approval_enabled: bool = False
        self.plan_risk_gate_enabled: bool = False

    def configure_web(
        self,
        *,
        sse_bridge_queue: asyncio.Queue[tuple[str, str]] | None,
        conversation_id: str | None,
        enabled: bool,
        plan_risk_gate: bool | None = None,
    ) -> None:
        self._sse_bridge_queue = sse_bridge_queue
        cid = (conversation_id or "").strip()
        self.conversation_id = cid or None
        self.web_approval_enabled = bool(
            enabled and self.conversation_id and sse_bridge_queue is not None
        )
        if plan_risk_gate is not None:
            self.plan_risk_gate_enabled = plan_risk_gate
    ...
    async def confirm_interactive(
        ...
        *,
        risk: PlanRiskAssessment | None = None,
    ) -> bool:
        if self.web_approval_enabled and self._sse_bridge_queue and self.conversation_id:
            from backend.agentic.safety import web_approval

            approval_id = web_approval.create_approval_id()
            fut = web_approval.register(self.conversation_id, approval_id)
            paths = [str(p) for p in confirmation.affected_paths]
            data = {
                "approval_id": approval_id,
                "conversation_id": self.conversation_id,
                "tool_name": confirmation.tool_name,
                "description": confirmation.description,
                "params_summary": summarize_params(dict(confirmation.params)),
                "command": confirmation.command,
                "affected_paths": paths,
                "risk_level": risk.level.name if risk else None,
                "risk_rationale": risk.rationale if risk else None,
            }
            payload = {"type": "approval_required", "data": data}
            line = f"data: {json.dumps(payload, ensure_ascii=False, default=str)}\n\n"
            await self._sse_bridge_queue.put(("side", line))
            return await fut

        return self.request_confirmation(confirmation)
