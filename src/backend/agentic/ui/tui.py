...
from backend.agentic.utils.paths import display_path_rel_to_cwd
import re

from backend.agentic.utils.text import truncate_text
...
class TUI:
    def __init__(
        ...
        self._reflection_status: Status | None = None

    def begin_reflection(self) -> None:
        """Affiche « Réflexion… » avec points animés jusqu’à ``end_reflection``."""
        if self._reflection_status is not None:
            return
        self._reflection_status = Status(
            "[dim]Réflexion[/dim][dim]…[/dim]",
            console=self.console,
            spinner="dots",
            spinner_style="dim",
            speed=0.9,
        )
        self._reflection_status.start()

    def end_reflection(self) -> None:
        if self._reflection_status is not None:
            self._reflection_status.stop()
            self._reflection_status = None
        ...
        for key, value in self._ordered_args(tool_name, args):
            ...
            if isinstance(value, str):
                if key in {"content", "old_string", "new_string"}:
                    ...
                    display = f"<{line_count} lines • {byte_count} bytes>"
                else:
                    display = value
    ...
    def tool_call_complete(
        ...
        elif name == "visualization_from_prompt" and success:
            output_display = truncate_text(
                output,
                self.config.model_name,
                self._max_block_tokens,
