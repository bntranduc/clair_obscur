...
from backend.agentic.agent.persistence import PersistenceManager, SessionSnapshot
from backend.agentic.agent.session import Session
from backend.agentic.config.config import ApprovalPolicy, Config
from backend.agentic.config.loader import load_config
from backend.agentic.ui.tui import TUI, get_console
...
class CLI:
    ...
    async def _process_message(self, message: str) -> str | None:
        ...
        self.tui.begin_reflection()
        try:
            async for event in self.agent.run(message):
                if event.type == AgentEventType.AGENT_STEP:
