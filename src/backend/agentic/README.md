...
- Sous-agent **remédiation IR** : `subagent_remediation_soc` — plan contain/eradicate/recover et vérifications ; logs / SQL / firewall + recherche web optionnelle (`tools/remediation_subagent.py`)
- Sous-agent **MITRE** : `subagent_mitre_simple` — référence ATT&CK via web (`web_search` / `web_fetch` uniquement, `tools/mitre_subagent.py`)
